...
#        if line.find('GC')>=0:
        ...
        if (str(time_stamp) in time_list) or (time_stamp>=int(time_list[0]) and time_stamp<int('1419984000', 0)) or time_stamp==(int(time_list[0])-1): 
            del time_list[0]
            time_list.append(str(int('0xffffffff', 0)))
	    if time_stamp==(int(time_list[0])-1):
