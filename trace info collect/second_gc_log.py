...
def second_gc_log(filename, gclog, cachefile, fullcachein):
    ...
    @param starting_point, used to read the first line as the starting time
    ...
    for line in fp:
        ...
        time_stamp=0
        if starting_point == 0:
            starting_time = int(line.split('\n')[0]) # it is already know first line is time stamp
            starting_point += 1
...
#        if line.find('GC')>=0:
        ...
        if (str(time_stamp) in time_list) or (time_stamp>=int(float(time_list[0])) and time_stamp<int('1419984000', 0)) or time_stamp==(int(float(time_list[0]))-1): 
            del time_list[0]
            time_list.append(str(int('0xffffffff', 0)))
            if time_stamp == (int(float(time_list[0]))-1):
